...
from smbop_utils import moz_sql_parser as msp

import smbop_utils.node_util as node_util
import smbop_utils.hashing as hashing
...
import numpy as np
import os
from smbop_utils.replacer import Replacer
import time
from smbop_utils.dataset_readers.enc_preproc import *
import smbop_utils.dataset_readers.disamb_sql as disamb_sql
# from smbop_utils.cache import TensorCache
...
class SmbopSpiderDatasetReader(DatasetReader):
    def __init__(
        ...
        dataset_path: str = "spider/database",
    ...
    def _read(self, file_path: str):
        if file_path.endswith(".json"):
            print(f"this runs for {file_path}")
...
if __name__ == "__main__":
    ...
    question_token_indexers = {"tokens": PretrainedTransformerIndexer(model_name="bert-base-cased")}

    reader = SmbopSpiderDatasetReader(question_token_indexers=question_token_indexers, tables_file="spider/tables.json")

    dataset = reader.read("spider/train_others.json")
    ...
    for i, instance in enumerate(dataset):
        ...
        print(f"Instance {i}:")
        print(instance)
        print(instance['tree_obj'].metadata)
        print(instance['entities'].metadata)
        print(instance['gold_sql'].metadata, len(instance['gold_sql'].metadata))
        for pre, _, node in RenderTree(instance['tree_obj'].metadata):
            print(f"{pre}{node.name}")
        print(instance['orig_entities'].metadata)
        dataset.__next__()
